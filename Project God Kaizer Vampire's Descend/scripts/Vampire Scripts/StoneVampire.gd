...
@onready var collider: CollisionPolygon2D = $AnimatedSprite2D/HitDetectionArea/Collider
...
@onready var contact_area: Area2D = $AnimatedSprite2D/HitDetectionArea
...
func chasing_state(delta):
	var effective_range = attack_range * 1.05

	if sprite and sprite.scale.x > 0:  # facing right
		effective_range *= 0.95
	if player and is_instance_valid(player):
