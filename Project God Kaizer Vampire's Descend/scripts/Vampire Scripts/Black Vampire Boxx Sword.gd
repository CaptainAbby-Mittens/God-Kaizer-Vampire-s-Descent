...
func _process(delta):
	...
	if sprite.animation != last_animation or sprite.frame != last_frame:
...
func attack():
	...
	var effective_range = attack_range * 1

	# If vampire is facing right, shrink the attack range
	if sprite and sprite.scale.x > 0:  # facing right
		effective_range *= 1.4  # reduce range (adjust this factor as needed)
